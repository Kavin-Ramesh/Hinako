...
const features = [
  ...
  },
  {
    icon: Feather,
    title: "Twelve grams",
    copy: "Lightweight and compact. Disappears in your palm and on your strap.",
  },
  {
    icon: Gem,
    title: "Premium build",
          ...
          {features.map((f, i) => {
            ...
            return (
              <Reveal key={f.title} delay={(i % 3) * 0.08} className="group">
                <div className="mb-6 inline-flex h-12 w-12 items-center justify-center rounded-md border border-mist text-cornflower transition-colors duration-500 group-hover:border-cornflower">
                  <Icon strokeWidth={1.2} className="h-5 w-5" />
                </div>
                <h3 className="font-display text-xl font-medium tracking-[-0.02em] text-ink md:text-2xl">
                  {f.title}
                </h3>
